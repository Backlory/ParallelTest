...
void sobel_filtering_03(FIBITMAP* imgIn, FIBITMAP* imgOut, bool Display)
{
...
*/
int sobelFunc(const char imgpath[], bool Display)
{
    ...
    sobel_filtering_03(imgIn, imgOut, Display);

    std::string fnIn(imgpath);
    std::size_t found = fnIn.find_last_of('.');
    std::string fnOut = fnIn.substr(0, found) + "_SobelSerial_FI" + fnIn.substr(found);
...
void MyTasker::runTest03_SobelSerial(bool Display) {
    if (Display) std::cout << "Test03_SobelSerial starts..." << std::endl;

    char imgpath[] = R"(F:\CPPprojs\221003-finalrepo\finalrepo\testimg.jpeg)";

    int out = sobelFunc(imgpath, Display);
